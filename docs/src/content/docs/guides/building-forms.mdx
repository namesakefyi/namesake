...
Add a new folder to `src/content/forms` with the following structure:
...
Now that your form collects data, go back to `src/content/pdfs` and update the resolver values in `index.ts` with the matching fields from `data`:
...
Add end-to-end tests to `e2e.spec.ts` covering the full form flow. Look at existing form specs in `src/content/forms` for examples of how to fill in steps, verify the review page, and confirm PDF output.
