...
```mermaid
---
title: "Namesake Forms: State Machine Flow"
---
stateDiagram-v2
    [*] --> Title
...
| Context | Description |
|---|---|
| `formSlug` | Set once when the machine is created |
| `currentStepId` | Set on `START` and updated on each `GOTO_STEP` while in `filling` |
| `editingStepId` | Set on `EDIT_STEP` when entering `editing`, then cleared on `SAVE_EDIT` |
...
The [`resolveFormVisibility`](https://github.com/namesakefyi/namesake/blob/main/web/src/lib/forms/formVisibility.ts#L82) function  takes an input of `steps`, user `formData`, and `pdfs`, and parses the visibility rules. It returns:
